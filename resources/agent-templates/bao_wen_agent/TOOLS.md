...
```
.{你的工作空间路径}/大纲.md                  # 大纲文件（章节列表；若用户未指定长度，默认生成 10 章）
.{你的工作空间路径}/设定/                    # 设定目录
.{你的工作空间路径}/设定/故事设定.md         # 故事设定（文章标题、主要事件、文本类型、世界观设定）（必须生成）
.{你的工作空间路径}/设定/角色设定.md         # 角色设定（主角信息、角色表、角色关系网）（必须生成）
.{你的工作空间路径}/设定/...                 # 其他设定文件
.{你的工作空间路径}/正文/                    # 正文目录
.{你的工作空间路径}/正文/导语.md             # 导语文件（开头吸引内容，500 字以内）
.{你的工作空间路径}/正文/第一章.md           # 第一章正文（默认 2000 字以内）
.{你的工作空间路径}/正文/第二章.md           # 第二章正文
.{你的工作空间路径}/正文/...                 # 其余章节
```
...
在执行以下任何操作之前，**必须**先向用户说明操作内容并明确征得同意，未获确认不得执行：

- **删除文件或目录**（`rm`、`rmdir`、`trash`、`del` 等命令）
- **修改系统环境**（安装/卸载软件、修改环境变量、写入注册表、更改系统配置文件）
- **网络发送**（发邮件、发消息、POST 请求到外部服务等任何"离开本机"的操作）
- **读取隐私信息**（浏览器 Cookie/历史记录、密码管理器、SSH 私钥、系统 Keychain、用户凭证文件）

**询问格式示例：**
> 我即将执行：`rm -rf ./正文/第一章.md`
> 这将永久删除该文件，是否确认？
