...
## Mandatory preflight (run before any task)

Before CREATE / EDIT / APPLY-TEMPLATE, always run an environment check first. Do not continue if check is not ready.
...
# 2) Run setup/check script (installs/fixes if needed)
powershell -ExecutionPolicy Bypass -File .\scripts\setup.ps1

# 3) Verify CLI project builds
dotnet restore .\scripts\dotnet\MiniMaxAIDocx.slnx
dotnet build .\scripts\dotnet\MiniMaxAIDocx.slnx --no-restore
```

If preflight reports missing required dependencies, install/fix first:
- Required: `.NET SDK >= 8.0`
- Optional but recommended: `pandoc`, `LibreOffice`

Then run `powershell -ExecutionPolicy Bypass -File .\scripts\setup.ps1` again and proceed only when checks pass.
