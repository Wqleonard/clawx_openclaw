...
Before READ / CREATE / EDIT / FIX / VALIDATE, always check runtime dependencies first.
...
# Required Python packages for analysis flows
python -c "import pandas, openpyxl"

# Optional dynamic recalculation capability
python .\scripts\libreoffice_recalc.py --check
```

If any required check fails, stop and guide the user to install dependencies before continuing:
- Required: Python 3
- Required for READ/analyze: `python -m pip install pandas openpyxl`
- Optional (recommended for dynamic formula recalculation): LibreOffice/soffice
