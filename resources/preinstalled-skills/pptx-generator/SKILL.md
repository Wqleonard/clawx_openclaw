...
Before read/edit/create operations, always verify environment first. Do not proceed when required dependencies are missing.
...
# Required for PPT generation
node -e "require('pptxgenjs'); console.log('pptxgenjs ok')"

# Recommended for content extraction / QA
python -m markitdown --help
```

If checks fail, install first and then re-check:
- Required for generation: `npm install -g pptxgenjs`
- Recommended for read/QA: `python -m pip install "markitdown[pptx]"`
- Optional icon helpers: `npm install -g react-icons react react-dom sharp`
