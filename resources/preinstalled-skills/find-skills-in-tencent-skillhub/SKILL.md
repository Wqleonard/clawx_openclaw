...
- **Linux / macOS**：按 [skillhub.md](https://skillhub-1388575217.cos.ap-guangzhou.myqcloud.com/install/skillhub.md) 安装 Skillhub CLI。

- **Windows**：在 PowerShell 中执行以下命令（可将 `DestinationPath` 改为你希望解压到的目录，如 `.\skillhub-openclaw` 表示当前目录下的 `skillhub-openclaw` 文件夹）：

  ```powershell
  # 1. 下载 Windows 版 skillhub 安装包
  Invoke-WebRequest -Uri "https://github.com/cnwpdb/skillhub-openclaw-windows/raw/main/dist/skillhub-windows-openclaw-v1.zip" -OutFile "skillhub-windows-openclaw-v1.zip"

  # 2. 解压到目标目录（示例：当前目录下的 skillhub-openclaw）
  Expand-Archive -Path .\skillhub-windows-openclaw-v1.zip -DestinationPath .\skillhub-openclaw -Force

  # 3. 以绕过执行策略方式运行安装脚本
  powershell -ExecutionPolicy Bypass -File .\skillhub-openclaw\install-skillhub-openclaw-windows.ps1
  ```

  安装完成后建议：
  1. 重启终端或刷新 PATH；
  2. 运行 `skillhub --help` 确认可用；
  3. 需要时可运行 `skillhub install <slug>` 安装技能（如 `skillhub install tavily-search`），并配置相应环境变量（如 `TAVILY_API_KEY`）；
  4. 使用 `skillhub search <关键词>` 搜索技能（如 `skillhub search calendar`）。
