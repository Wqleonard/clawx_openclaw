...
function emitChannelEvent(
  ...
  if (channelType === UI_WECHAT_CHANNEL_TYPE) {
    logger.info('[WECHAT_QR_DEBUG] emit channel event', {
      eventName,
      event,
      payloadType: typeof payload,
      hasMainWindow: Boolean(ctx.mainWindow && !ctx.mainWindow.isDestroyed()),
    });
  }
...
export async function handleChannelRoutes(
  ...
  if (url.pathname === '/api/channels/wechat/start' && req.method === 'POST') {
    try {
      ...
      logger.info('[WECHAT_QR_DEBUG] start request', { requestedAccountId });

      const installResult = await ensureWeChatPluginInstalled();
      logger.info('[WECHAT_QR_DEBUG] plugin install result', installResult);
      ...
      logger.info('[WECHAT_QR_DEBUG] login start result', {
        hasQrcodeUrl: Boolean(startResult.qrcodeUrl),
        hasSessionKey: Boolean(startResult.sessionKey),
      ...
      logger.error('[WECHAT_QR_DEBUG] start request failed', error);
      sendJson(res, 500, { success: false, error: String(error) });
    }
    return true;
  }
